...
本リポジトリの成果物は Agent Skill（`SKILL.md`）であり、実行されるアプリケーションを持ちません。したがって検証の対象は「`SKILL.md` が仕様どおりの内容と構造を持つか」になります。

| 項目 | 選定 | 理由 |
| --- | --- | --- |
| テストメソッド | 単体テスト（契約テスト） | 検証対象が単一ファイルの構造と記載内容であり、モジュール間連携を持たないため |
| テストメソッド | ブラックボックステスト | `SKILL.md` を入力、抽出した見出し・フロントマター・記載事項を出力として扱うため |
| フレームワーク | Jest | 対象言語が JavaScript であり、CI が既に Node.js を使用しているため |
| 補助ライブラリ | js-yaml | YAML フロントマターの解釈に用いる。自前の実装を持たないため |

E2E テスト（Playwright）は採用しません。ブラウザで操作する画面を持たないためです。
...
テストの配置は `test/README.md` を参照してください。
