...
| P8 | シーン実装とレンダリング | 構成データを描画するシーンを実装し、画面内収まりを機械的に検査してからCLIでレンダリングする | 検査を通過し、mp4が出力仕様どおりに生成されている |
...
| P8 | 画面内収まりの検査で不適合 | レンダリングへ進まず、シーン実装の修正へ戻る |
...
レンダリングの前に、各シーンの代表フレームを評価し、次の3件を機械的に検査する。要素の描画矩形と算出フォントサイズの比較で判定できるため、目視を要しない。

| 記号 | 検査 | 不適合とする条件 |
|---|---|---|
| I1 | 安全余白のはみ出し | 描画矩形が上下左右5%の安全余白の外へ出ている |
| I2 | 親要素からの欠け | 親要素の内寸を超えて配置され、視覚的に欠ける |
| I3 | 文字サイズの下限 | 算出フォントサイズが縦解像度の3%（1920x1080で32px）を下回る |

不適合を検出した場合はレンダリングへ進まず、シーン実装の修正へ戻る。この検査は10.3のうちR1・R2に相当する不適合の絞り込みであり、検査の通過をもってR1・R2の適合と見なさない。
...
設置先 `~/.claude/skills/spec-to-mp4-video` を占有する経路が2通りある。

| 経路 | 想定利用者 | 設置物 |
|---|---|---|
| 配布zipの展開 | 利用者 | `SKILL.md` の実体（コピー） |
| symlink設置 | 開発者 | リポジトリへのsymlink |

両者は同一のパスを占有するため衝突する。実体が置かれた状態でもSKILL.mdは読めるため、設置に失敗したまま古い版を読み続ける事故が起きる。これを避けるため、次を定める。

- 設置の前に既存の有無と種別（none／symlink／実体）を判定し、実体である場合はその所在を報告する。
- 実体がある状態でsymlink設置を行わない。上書きせず、既存の所在を提示して指示を待つ。既存の移動・改名・削除を自動で行わない。
- 設置の後に、設置物の種別と参照先を確認する。あわせて、設置先から読み込まれるSKILL.mdがリポジトリの版と一致するかを確認する。
- 利用者向けのzip展開手順は変更しない。対処は開発者側の設置手順に寄せる。
