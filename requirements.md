...
| 項目 | 値 |
|---|---|
| SKILL.md 想定行数 | 約200行（上限目安500行に対して余裕を持つ） |
...
必須項目の欠落を検出した場合、処理を停止し、欠落項目を列挙して提示する。文言・尺・シーン構成をスキル側で創作しない。想定尺のみが欠けている場合は第9節の尺下限式による算出値を提示し、承認を得たうえで再開する。
...
| 生成物 | 役割 |
|---|---|
| 構成データファイル | 仕様書を正規化したシーン定義。画面文言の唯一の保持先 |
...
手順を7段で構成する。各段は完了条件を持ち、完了条件を満たさない状態で次段へ進まない。

| 段 | 名称 | 処理 | 完了条件 |
|---|---|---|---|
| P1 | 受理判定 | 第6節の条件で発火可否を判定する | C1・C2・C3が成立している |
| P2 | 読み取りと充足判定 | 仕様書を読み、必須項目の充足を判定する | 必須項目がすべて揃っている |
| P3 | 規模判定 | シーン数・総尺がデモ版の範囲内かを判定する | 範囲内である |
| P4 | 構成データ生成 | 仕様書を正規化し、シーン定義を構成データファイルへ書き出す | 画面文言が原文のまま保持され、各シーンの尺が下限式を満たす |
| P5 | 環境準備 | 動画生成プロジェクトを構築し、日本語フォントの可用性を判定する | プロジェクトが起動し、日本語グリフが欠落しない |
| P6 | シーン実装とレンダリング | 構成データを描画するシーンを実装し、CLIでレンダリングする | mp4が出力仕様どおりに生成されている |
| P7 | 表示検証 | 第10節の要件を満たすかを判定する | 全要件を満たす |
...
| P3 | 範囲超過 | 停止し、対象範囲の指定を求める |
| P4 | 指定尺が下限を下回る | 下限値を採用し、差分を報告する |
| P5 | 日本語フォントが不足 | 停止し、必要なフォントの導入を求める |
| P7 | 判読性・はみ出しの不適合 | P6へ差し戻す |
| P7 | 尺の不足 | P4へ差し戻す |
...
上限を超える仕様書は、先頭からの部分生成を行わず、対象範囲の明示的な指定を待つ。

### 9.3 尺の下限式

各シーンの尺の下限は、画面表示テキストの総文字数を毎秒6文字で除した値に、表示前後の余白2秒を加えた値とする。仕様書の指定尺がこの下限を下回る場合、下限値を採用する。
...
- 仕様書の画面文言を要約・言い換え・翻訳しない。原文をそのまま採用する。
...
flowchart TD
    ...
    P2 -->|充足| P3[P3 規模判定]
    P3 -->|上限超過| S2[停止：対象範囲の指定を求める]
    P3 -->|範囲内| P4[P4 構成データ生成]
    P4 -->|指定尺が下限未満| P4B[下限値を採用し差分を報告]
    P4B --> P5[P5 環境準備]
    P4 -->|下限を満たす| P5
    P5 -->|日本語フォント不足| S3[停止：フォント導入を求める]
    P5 -->|可用| P6[P6 シーン実装とレンダリング]
    P6 --> P7[P7 表示検証]
    P7 -->|判読性・はみ出しが不適合| P6
    P7 -->|尺が不足| P4
    P7 -->|全要件を満たす| R[完了報告：出力パス・シーン一覧・総尺]
```
