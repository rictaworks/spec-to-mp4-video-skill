...
画面に表示する文言そのものは必須項目としない。文言は設計の産物ではなく制作の産物であり、設計レベルの文書には含まれないことがあるためである。文言は第9節P4の台本生成で確定する。仕様書に画面表示テキストが記載されている場合は、P4でそれを原文のまま採用する。
...
| P3 | 情報収集 | 逐語一致を要する文字列を、出所を特定できる情報源から集める | 逐語指定した文字列すべてに出所が紐づいている |
| P4 | 台本生成 | シーンの主張から画面に表示する文言を起こし、承認を得る | 全シーンの画面表示テキストが確定し、承認を得ている |
| P5 | 規模判定 | シーン数・総尺がデモ版の範囲内かを判定する | 範囲内である |
...
| P3 | 逐語一致を要する文字列の出所を特定できない | 停止し、対象を列挙して正しい値または資料を求める |
...
flowchart TD
    ...
    P2 -->|充足| P3[P3 情報収集]
    P3 -->|出所を特定できない| S4[停止：正しい値または資料を求める]
    P3 -->|出所が揃う| P4[P4 台本生成]
    P4 -->|承認が得られない| S5[停止：台本を提示し承認を待つ]
    P4 -->|承認を得た| P5[P5 規模判定]
    P5 -->|上限超過| S2[停止：対象範囲の指定を求める]
